...
class TaskList extends StatefulWidget {
  ...
  TaskList(this._flat, this.user);

  @override
  State<StatefulWidget> createState() {
    return TaskListState(this._flat, this.user);
  }
}
...
class TaskListState extends State<TaskList> {
              ...
              builder: (context, AsyncSnapshot<QuerySnapshot> taskSnapshot) {
                ...
                if (sortAscending) {
                  if(sortField == 'nextDueDate') {
                    tasks.sort((Task a, Task b) => a.getNextDueDate().compareTo(b.getNextDueDate()));
                  }
                } else {
                  tasks.sort((Task a, Task b) => b.getNextDueDate().compareTo(a.getNextDueDate()));
                }

                if (!peopleFilterAllSelected) {
                  tasks.removeWhere((Task s) =>
                      !s.getAssignees().contains(_userId.trim()));
                }
                ...
                if (!_isResponsibility) {
                  ...
                  tasks.removeWhere(
                      (Task s) => s.getType().toString().trim() == 'Reminder');
                }

                if (!_isIssue) {
                  debugPrint("Removing complaint");
                  tasks.removeWhere(
                      (Task s) => s.getType().toString().trim() == 'Complaint');
                }

                if (!_isPayment) {
                  debugPrint("Removing payment");
                  tasks.removeWhere(
                      (Task s) => s.getType().toString().trim() == 'Payment');
                }
                ...
                return new ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int position) {
                    ...
                    DateTime datetime = task.getNextDueDate() ==
                            null
                        ? getNextDueDateTime(
                            DateTime.now(),
                            task.getDue().toDate(),
                          ...
                          Slidable(
                            ...
                            actions: <Widget>[
                              new IconSlideAction(
                                ...
                                icon: Icons.check,
                                onTap: () async {
                                  TaskHistory taskHistory = new TaskHistory(_userId, _userName);
                                  int _repeat = task.getRepeat();
                                  if (_repeat != -1 && task.getType() != 'Complaint') {
                                  ...
                                  child: ListTile(
                                    ...
                                    leading: Container(
                                      child: Tooltip(
                                        ...
                                        message: task.getType(),
                                        child: IconButton(
                                          icon: Icon(
                                            (icons[task.getType()]['icon']),
                                            color: (icons[task.getType()]['color']),
                                          ),
                                          onPressed: () {
                                            dynamic tooltip =
                                                tooltipKey[position]
  ...
  DateTime getNextDueDateTime(
      DateTime nowDueDate, DateTime due, int repeat, List<int> frequency) {
  ...
  void navigateToAddTask(String typeOfTask, {taskId}) {
    Navigator.push(
      ...
      MaterialPageRoute(builder: (context) {
        return CreateTask(taskId, this._flat, typeOfTask, this.user);
      }),
    );
  }
  ...
  void navigateToViewTask({taskId}) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) {
        return ViewTask(taskId, this._flat, this.user);
      }),
    );
  }
